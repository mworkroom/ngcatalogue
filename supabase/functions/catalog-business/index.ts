...
async function handleLogin(body: Record<string, unknown>) {
  const configuredCode = Deno.env.get("BUSINESS_ACCESS_CODE")?.trim() ?? "";
  const submittedCode = String(body.code ?? "").trim();

  if (!/^\d{8}$/.test(configuredCode)) {
    console.error("BUSINESS_ACCESS_CODE is not configured as an 8-digit value.");
    return json({ error: "Business access is not configured." }, 500);
  }
  ...
  const now = Date.now();
  const expiresAt = new Date(now + BUSINESS_SESSION_DAYS * 24 * 60 * 60 * 1000);
  const sessionSecret = getSessionSecret();

  if (!sessionSecret) {
    console.error("BUSINESS_SESSION_SECRET is not configured.");
    return json({ error: "Business access is not configured." }, 500);
  }

  const token = await new SignJWT({ role: "business" })
...
async function handleProducts(body: Record<string, unknown>) {
  ...
  if (error) {
    console.error("Business product load failed.", error.message);
...
function getSessionSecret() {
  const secret = Deno.env.get("BUSINESS_SESSION_SECRET")?.trim() ?? "";
